...
from libs.modules.conv import StreamableConv1d, StreamableConvTranspose1d
...
class SEANetResnetBlock(nn.Module):
    ...
    def __init__(
        ...
        hidden = dim // compress
        block = []
        for i, (kernel_size, dilations) in enumerate(zip(kernel_size, dilations)):
            in_chs = dim if i == 0 else hidden
            out_chs = dim if i == len(kernel_size) - 1 else hidden
            block += [
...
class SEANetEncoderKeepDimension(nn.Module):
    """
