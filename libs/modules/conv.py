...
def get_extra_padding_for_conv2d(
    ...
    return extra_padding_height, extra_padding_width
...
def unpad2d(x: torch.Tensor, paddings: tp.Tuple[int, int, int, int]):
    ...
    assert padding_top >= 0 and padding_bottom >= 0 and padding_left >= 0 and padding_right >= 0, (
        padding_top, padding_bottom, padding_left, padding_right)
    assert (padding_top + padding_bottom) <= x.shape[-2], f"Invalid top/bottom padding: {padding_top}/{padding_bottom}"
    assert (padding_left + padding_right) <= x.shape[-1], f"Invalid left/right padding: {padding_left}/{padding_right}"
...
class NormConv2d(nn.Module):
    ...
    def __init__(
        ...
        self.norm = get_norm_module(self.conv, causal, "BatchNorm2d", **norm_kwargs)
        self.norm_type = norm
...
class DownConvBlock(nn.Module):
    ...
    def __init__(
        ...
        if stride > 1 and dilation > 1:
            warnings.warn(
                "Streamable has been initialized with stride > 1 and dilation > 1"
    ...
    def forward(self, x):
        ...
        extra_padding_height, extra_padding_width = get_extra_padding_for_conv2d(
        ...
        if self.causal:
            ...
            x = pad2d(x, (padding_total, extra_padding, ), mode=self.pad_mode)
        else:
...
class NormConvTranspose1d(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.norm = get_norm_module(self.convtr, causal, "BatchNorm2d", **norm_kwargs)
        self.norm_type = norm
...
class StreamableConvTranspose1d(nn.Module):
    ...
    def forward(self, x):
        ...
        # We will only trim fixed padding. Extra padding from 'pad_for_conv1d' would be removed
        # at the very end, when keeping only the right length for the output,
